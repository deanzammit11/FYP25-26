...
def run_xgboost(data_path = "data/features/eng1_data_combined.csv"):
    ...
    for fold_season in fold_seasons: # For each fold season
        ...
        fold_model.fit(X_train_selected.iloc[fold_train_idx], y_train.iloc[fold_train_idx]) # The model for that specific fold is trained only on data from seasons before the prediction year where y_train is already shifted
        ...
        fold_out["Predicted"] = [int(prediction) - 1 for prediction in fold_predictions] # Predicted outcomes are converted back from 0, 1 and 2 to -1, 0 and 1 and added to the validation rows dataframe
        fold_out["ProbHome"] = fold_probabilities[:, fold_classes.index(2)] # Home win probability is added to the validation rows dataframe
        fold_out["ProbDraw"] = fold_probabilities[:, fold_classes.index(1)] # Draw probability is added to the validation rows dataframe
        fold_out["ProbAway"] = fold_probabilities[:, fold_classes.index(0)] # Away win probability is added to the validation rows dataframe
        season_path = f"data/results/xgboost/xgboost_{fold_season}_predictions.csv" # The path where to store the predictions csv is stored
